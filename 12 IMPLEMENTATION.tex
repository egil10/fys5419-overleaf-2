...
The implementation is a small collection of pure-NumPy / SciPy modules, deliberately written from scratch rather than on top of a high-level VQA library so that every conventional choice (sign of the cost, qubit ordering, scaling of the approximation ratio) is explicit and auditable. The package layout mirrors the pipeline introduced in \cref{section:methods}.

\paragraph{Modules.} The \emph{data layer} (\texttt{data.py}) loads the cached daily-close Parquet files and assembles $(\boldsymbol{\mu},\boldsymbol{\Sigma})$. The \emph{problem layer} (\texttt{portfolio.py}, \texttt{ising.py}) fixes the single canonical cost $C(\boldsymbol{x})$ through the \texttt{PortfolioProblem} dataclass and the \texttt{DEFAULTS} dictionary used by every notebook, and performs the $x_i = (1-z_i)/2$ substitution to build the diagonal $H_C$ and sparse mixer $H_M$. The \emph{solver layer} (\texttt{qaoa.py}, \texttt{optimize.py}, \texttt{classical.py}) implements the alternating ansatz with its $\langle\psi|H_C|\psi\rangle$ energy, the classical outer loop, and the brute-force, greedy-Sharpe, Markowitz-then-round and simulated-annealing baselines behind a uniform \texttt{SolverResult} type. Finally, the \emph{orchestration layer} (\texttt{metrics.py}, \texttt{compare.py}, \texttt{plotting.py}) computes the approximation ratio together with $p_{\text{opt}}$ and $p_{\text{feas}}$, then drives the experiments and figures in \cref{section:result}.
