...
What does the variational energy surface $E(\gamma,\beta)$ of \eqref{eq:Egb} actually look like? Mapping it on a grid is only feasible at $p=1$, where the surface is two-dimensional; \cref{fig:landscape} shows the contour plot on the canonical $n=16$ instance.
...
The surface is visibly $\pi$-periodic in $\gamma$ and exhibits a small number of broad basins separated by saddles; the energy varies by roughly two orders of magnitude across the grid, from $E \sim 50$ in the bright hot-spots near $(\gamma/\pi, \beta/\pi) \approx (0, 0.25)$ to the small basins in the corners where the COBYLA optimum lives. Two practical consequences follow. First, the basins of attraction are reasonably large, so random multi-start COBYLA does find the right neighbourhood at $p=1$, consistent with the single-run figure above. Second, at $p \geq 2$ the analogue surface lives in $\mathbb{R}^4$ and beyond, and the same multi-start budget covers it exponentially less densely --- a useful frame for reading the non-monotonic depth sweep in the next subsection.
